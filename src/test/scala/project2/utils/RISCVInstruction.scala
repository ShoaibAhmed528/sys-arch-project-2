...
final case class RISCVInstruction(
    instruction: String,
    assembly: UInt,
    effect: RVFI,
...
object RISCVInstruction {
  ...
  def getRandomImmediate(bits: Int): Int = {
    val max = 1 << bits
    MyRandom.nextLong(max).toInt
  }

  def getRandomRegister(may_be_zero: Boolean): Int = {
    if (may_be_zero) MyRandom.nextLong(32).toInt
    else MyRandom.nextLong(31).toInt + 1
  }

  def getRandomMemoryValue(): Long = {
    MyRandom.nextLong(1L << 32)
  }
  ...
  def getADDI(state: ProcessorState): RISCVInstruction = {
    ...
    val instruction =
      "addi x" + rd + " x" + rs1 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
      ...
      rs2_rdata = 0.U
      rd_addr = rd.U
      rd_wdata = signedToUnsigned(
        state.registers(rs1).toLong + getSignedValue(imm, imm_width),
        32
      ).U
      pc_rdata = state.pc.U
  ...
  def getSLTI(state: ProcessorState): RISCVInstruction = {
    ...
    val rs1 = getRandomRegister(true)
    val instruction =
      "slti x" + rd + " x" + rs1 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
      ...
      rd_wdata =
        if (
          unsignedToSigned(state.registers(rs1).toLong, 32) < getSignedValue(
            imm,
            imm_width
          )
        ) 1.U
  ...
  def getSLTIU(state: ProcessorState): RISCVInstruction = {
    ...
    val rs1 = getRandomRegister(true)
    val instruction =
      "sltiu x" + rd + " x" + rs1 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
      ...
      rd_wdata =
        if (
          state.registers(rs1) < signedToUnsigned(
            getSignedValue(imm, imm_width),
            32
          )
        ) 1.U
        else 0.U
  ...
  def getANDI(state: ProcessorState): RISCVInstruction = {
    ...
    val rs1 = getRandomRegister(true)
    val instruction =
      "andi x" + rd + " x" + rs1 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
      ...
      rd_addr = rd.U
      rd_wdata = signedToUnsigned(
        state.registers(rs1).toLong & signedToUnsigned(
          getSignedValue(imm, imm_width),
          32
        ),
        32
  ...
  def getORI(state: ProcessorState): RISCVInstruction = {
    ...
    val rs1 = getRandomRegister(true)
    val instruction =
      "ori x" + rd + " x" + rs1 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
  ...
  def getXORI(state: ProcessorState): RISCVInstruction = {
    ...
    val rs1 = getRandomRegister(true)
    val instruction =
      "xori x" + rd + " x" + rs1 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
  ...
  def getSLLI(state: ProcessorState): RISCVInstruction = {
    ...
    val rs1 = getRandomRegister(true)
    val instruction =
      "slli x" + rd + " x" + rs1 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
  ...
  def getSRLI(state: ProcessorState): RISCVInstruction = {
    ...
    val rs1 = getRandomRegister(true)
    val instruction =
      "srli x" + rd + " x" + rs1 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
  ...
  def getSRAI(state: ProcessorState): RISCVInstruction = {
    ...
    val rs1 = getRandomRegister(true)
    val instruction =
      "srai x" + rd + " x" + rs1 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
      ...
      rd_addr = rd.U
      rd_wdata = signedToUnsigned(
        getSignedValue(state.registers(rs1).toLong, 32) >> imm,
        32
      ).U
      pc_rdata = state.pc.U
      pc_wdata = (state.pc + 4).U
  ...
  def getADD(state: ProcessorState): RISCVInstruction = {
    ...
    val effect = new RVFI {
      ...
      rd_addr = rd.U
      rd_wdata = signedToUnsigned(
        state.registers(rs1).toLong + state.registers(rs2).toLong,
        32
      ).U
      pc_rdata = state.pc.U
      pc_wdata = (state.pc + 4).U
  ...
  def getSLT(state: ProcessorState): RISCVInstruction = {
    ...
    val effect = new RVFI {
      ...
      rd_wdata =
        if (
          unsignedToSigned(state.registers(rs1).toLong, 32) < unsignedToSigned(
            state.registers(rs2).toLong,
            32
          )
        ) 1.U
        else 0.U
  ...
  def getSLL(state: ProcessorState): RISCVInstruction = {
    ...
    val effect = new RVFI {
      ...
      rd_addr = rd.U
      rd_wdata = signedToUnsigned(
        state.registers(rs1).toLong << (state.registers(rs2).toInt & 0x1f),
        32
      ).U
      pc_rdata = state.pc.U
      pc_wdata = (state.pc + 4).U
  ...
  def getSRA(state: ProcessorState): RISCVInstruction = {
    ...
    val effect = new RVFI {
      ...
      rd_addr = rd.U
      rd_wdata = signedToUnsigned(
        getSignedValue(state.registers(rs1).toLong, 32) >> (state
          .registers(rs2)
          .toInt & 0x1f),
        32
      ).U
  ...
  def getJAL(state: ProcessorState): RISCVInstruction = {
    val imm_width = 20
    val imm =
      (getRandomImmediate(imm_width) / 4) * 4 // Ensure imm is a multiple of 4
    val rd = getRandomRegister(false)
    ...
    val effect = new RVFI {
      ...
      pc_rdata = state.pc.U
      pc_wdata = signedToUnsigned(
        (state.pc + getSignedValue(imm, imm_width)) & ~0x1,
        32
      ).U
    }
    RISCVInstruction(instruction, assembly, effect)
  ...
  def getJALR(state: ProcessorState): RISCVInstruction = {
    val imm_width = 12
    var imm = getRandomImmediate(imm_width)
    val rd = getRandomRegister(false)
    val rs1 = getRandomRegister(true)
    // Ensure imm is a multiple of 4
    imm = imm - (imm + state.registers(rs1).toInt) % 4
    val instruction =
      "jalr x" + rd + " x" + rs1 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
  ...
  def getBEQ(state: ProcessorState): RISCVInstruction = {
    ...
    val instruction =
      "beq x" + rs1 + " x" + rs2 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
  ...
  def getBNE(state: ProcessorState): RISCVInstruction = {
    ...
    var imm_string = "0x" + imm.toHexString
    val instruction =
      "bne x" + rs1 + " x" + rs2 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
  ...
  def getBLT(state: ProcessorState): RISCVInstruction = {
    ...
    var imm_string = "0x" + imm.toHexString
    val instruction =
      "blt x" + rs1 + " x" + rs2 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
  ...
  def getBLTU(state: ProcessorState): RISCVInstruction = {
    ...
    var imm_string = "0x" + imm.toHexString
    val instruction =
      "bltu x" + rs1 + " x" + rs2 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
  ...
  def getBGE(state: ProcessorState): RISCVInstruction = {
    ...
    var imm_string = "0x" + imm.toHexString
    val instruction =
      "bge x" + rs1 + " x" + rs2 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
  ...
  def getBGEU(state: ProcessorState): RISCVInstruction = {
    ...
    val rs2 = getRandomRegister(true)
    val instruction =
      "bgeu x" + rs1 + " x" + rs2 + getImmediateAsString(imm, imm_width)
    val assembly =
      BigInt(RISCVAssembler.fromString(instruction).split("\n")(0), 16).U
    val effect = new RVFI {
      valid = true.B
  ...
  def getLB(state: ProcessorState): RISCVInstruction = {
    ...
    val rd = getRandomRegister(false)
    val rs1 = getRandomRegister(true)
    val instruction = "lb x" + rd + " " + getImmediateAsString(
      imm,
      imm_width
    ) + "(x" + rs1 + ")"
    val assembly =
    ...
    val mem_value = (value & 0xff) << (offset.intValue * 8)
    ...
    val effect = new RVFI {
      ...
      rs2_rdata = 0.U
      rd_addr = rd.U
      rd_wdata = signedToUnsigned(getSignedValue(value, 8), 32).U
      pc_rdata = state.pc.U
      pc_wdata = (state.pc + 4).U
      mem_addr = signedToUnsigned(address, 32).U
      mem_rdata = signedToUnsigned(value, 8).U
      mem_rmask = 0x1.U
      mem_wdata = 0.U
      mem_wmask = 0.U
    }
  ...
  def getLH(state: ProcessorState): RISCVInstruction = {
    ...
    val rd = getRandomRegister(false)
    val rs1 = getRandomRegister(true)
    val instruction = "lh x" + rd + " " + getImmediateAsString(
      imm,
      imm_width
    ) + "(x" + rs1 + ")"
    val assembly =
    ...
    if (offset == 3) {
      ...
      val mem_value = (value & 0xffff) << (offset.intValue * 8)
      state.data_mem(BigInt(signedToUnsigned(address, 32)) - offset) = mem_value
    }
    val effect = new RVFI {
      valid = true.B
      ...
      rs2_rdata = 0.U
      rd_addr = rd.U
      rd_wdata = signedToUnsigned(getSignedValue(value, 16), 32).U
      pc_rdata = state.pc.U
      pc_wdata = (state.pc + 4).U
      mem_addr = signedToUnsigned(address, 32).U
      mem_rdata = signedToUnsigned(value, 16).U
      mem_rmask = 0x3.U
      mem_wdata = 0.U
      mem_wmask = 0.U
    }
  ...
  def getLW(state: ProcessorState): RISCVInstruction = {
    ...
    val rd = getRandomRegister(false)
    val rs1 = getRandomRegister(true)
    val instruction = "lw x" + rd + " " + getImmediateAsString(
      imm,
      imm_width
    ) + "(x" + rs1 + ")"
    val assembly =
    ...
    if (offset != 0) {
      ...
      if (offset == 1) {
        state.data_mem(BigInt(signedToUnsigned(address, 32)) - 1) =
          (mem_value_4 << 8) + (mem_value_3 << 16) + (mem_value_2 << 24)
        state.data_mem(BigInt(signedToUnsigned(address, 32)) + 3) = mem_value_1
      } else if (offset == 2) {
        state.data_mem(BigInt(signedToUnsigned(address, 32)) - 2) =
          (mem_value_4 << 16) + (mem_value_3 << 24)
        state.data_mem(BigInt(signedToUnsigned(address, 32)) + 2) =
          (mem_value_1 << 8) + mem_value_2
      } else {
        state.data_mem(BigInt(signedToUnsigned(address, 32)) - 3) =
          mem_value_4 << 24
        state.data_mem(BigInt(signedToUnsigned(address, 32)) + 1) =
          (mem_value_1 << 16) + (mem_value_2 << 8) + mem_value_3
      }
    } else {
  ...
  def getLBU(state: ProcessorState): RISCVInstruction = {
    ...
    val rd = getRandomRegister(false)
    val rs1 = getRandomRegister(true)
    val instruction = "lbu x" + rd + " " + getImmediateAsString(
      imm,
      imm_width
    ) + "(x" + rs1 + ")"
    val assembly =
  ...
  def getLHU(state: ProcessorState): RISCVInstruction = {
    ...
    val rd = getRandomRegister(false)
    val rs1 = getRandomRegister(true)
    val instruction = "lhu x" + rd + " " + getImmediateAsString(
      imm,
      imm_width
    ) + "(x" + rs1 + ")"
    val assembly =
  ...
  def getSB(state: ProcessorState): RISCVInstruction = {
    ...
    val rs1 = getRandomRegister(true)
    val rs2 = getRandomRegister(true)
    val instruction = "sb x" + rs2 + " " + getImmediateAsString(
      imm,
      imm_width
    ) + "(x" + rs1 + ")"
    val assembly =
    ...
    val effect = new RVFI {
      ...
      mem_rdata = 0.U
      mem_rmask = 0.U
      mem_wdata = signedToUnsigned(value, 8).U
      mem_wmask = 0x1.U
    }
    RISCVInstruction(instruction, assembly, effect)
  }
  ...
  def getSH(state: ProcessorState): RISCVInstruction = {
    ...
    val rs1 = getRandomRegister(true)
    val rs2 = getRandomRegister(true)
    val instruction = "sh x" + rs2 + " " + getImmediateAsString(
      imm,
      imm_width
    ) + "(x" + rs1 + ")"
    val assembly =
    ...
    val effect = new RVFI {
      ...
      mem_rdata = 0.U
      mem_rmask = 0.U
      mem_wdata = signedToUnsigned(value, 16).U
      mem_wmask = 0x3.U
    }
    RISCVInstruction(instruction, assembly, effect)
  }
  ...
  def getSW(state: ProcessorState): RISCVInstruction = {
    ...
    val rs1 = getRandomRegister(true)
    val rs2 = getRandomRegister(true)
    val instruction = "sw x" + rs2 + " " + getImmediateAsString(
      imm,
      imm_width
    ) + "(x" + rs1 + ")"
    val assembly =
