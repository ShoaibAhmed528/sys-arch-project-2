...
class SimplePackedUnit extends AbstractExecutionUnit {
  ...
  val pli_h_imm_raw =
    Cat(instr(15), instr(24, 16)) // we concatenate sign bit with rest.
  val pli_h_imm16 =
    Cat(Fill(6, pli_h_imm_raw(9)), pli_h_imm_raw) // then we extend it.
...
//psub.h
  ...
  when(io.valid) {
    ...
    io_reg.reg_rs1 := Mux(
      instr_type === RISCV_TYPE.pli_b || instr_type === RISCV_TYPE.pli_h || instr_type === RISCV_TYPE.plui_h,
      0.U,
      io.instr(19, 15)
    )
    io_reg.reg_rs2 := Mux(
